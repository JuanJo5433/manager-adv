...
import { createProcess, getProcesses } from "@/services/processes/processesServices";
...
const ProcesosPage: React.FC = () => {
  ...
  const transformProcess = (p: any): Process => {
    return {
      ...
      userId: p.userId || "",
      // Si no se proporciona el usuario, se asigna un objeto con valores por defecto.
      // Ajusta estos defaults según las necesidades reales de tu aplicación.
      user: p.user || {
        id: "",
        email: "",
        password: "",
  ...
  useEffect(() => {
    const fetchProcesses = async () => {
      try {
        ...
        const completeProcesses: Process[] = resultProcesses.map(transformProcess);
                  ...
                  {processes.map((proceso) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-gray-600">
                        ...
                        >
                          {proceso.status === 1 ? "En progreso" : "Completado"}
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-gray-600">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-gray-600">
                        <div className="text-blue-800">
                          <Link href={`processes/board/${proceso.id}`}>
                            <FaRegFolder />
                          </Link>
                        </div>
                      </td>
